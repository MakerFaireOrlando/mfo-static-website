...
| `schedule-stages.json` | The category dropdown on `/schedule/` (generated by the producer dashboard). | below |
...
## `schedule-stages.json` — the category dropdown

Also **generated**, and published in the same pull request as `schedule.json`.
`pages/schedule.md` loops over it to build the "Filter by category" menu, so a
stage added in the builder reaches the dropdown by publishing and nothing has to
be typed into the page.

```json
[
  { "value": "main-stage", "label": "Main Stage" },
  { "value": "power-racing-track", "label": "Power Racing" }
]
```

- `value` is the slug of the event's **`location`**, because that is the class
  each row on the page carries (`location | slugify`). It is never the slug of
  the label — an option built from "Power Racing" would match no rows on a
  stage named "Power Racing Track".
- `label` is what a visitor reads, which is what lets that stage offer itself by
  the shorter name.
- The order is the builder's, and an empty list is legitimate: the dropdown then
  offers only "show all".
- It is a **separate file** from `schedule.json` on purpose. Four pages iterate
  `site.data.schedule` as a bare list, so the stages cannot live inside it
  without breaking all four.

Editing it by hand works, but the next publish from the dashboard overwrites it
— change the stages in the builder instead.
